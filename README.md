...
- RCI戦略: 順位相関指数による売買
- エンベロープ戦略: 移動平均からの乖離率
- 3本移動平均: 短期・中期・長期MAの完全整列判定

**高度な戦略（31種）**
- モメンタム系: モメンタム、MACD、ストキャスティクス、ROC、TRIX（トリプルEMA）
- トレンド系: ADX、DMI、スーパートレンド、平均足、ドンチャンチャネル、パラボリックSAR、KAMA（適応型移動平均）、DEMA（二重EMA）
- ボラティリティ系: ボリンジャースクイーズ、ATRトレーリングストップ、ケルトナーチャネル、シャンデリアエグジット
- オシレーター系: ウィリアムズ%R、CCI、MFI（マネーフローインデックス）
- 出来高系: エルダーのフォースインデックス、CMF（チェイキン・マネーフロー）、出来高急増
- トレンドフォロー系: タートルトレーディング（伝説的システム）
- その他: 平均回帰、ブレイクアウト、ギャップ、チョピネス指数、アルーン指標、EMAリボン、ピボットポイント、フィボナッチリトレースメント、週次ピボットポイント
...
#### 🎉 大型アップデート：10戦略追加（42 → 52戦略）

**効果⭐⭐⭐⭐⭐ の戦略（3種）**
- **TRIX（トリプルEMA）**: 3重指数移動平均の変化率でノイズを除去、シグナルラインとのクロスオーバーで売買
- **CMF（チェイキン・マネーフロー）**: 価格と出来高から資金フローを測定、機関投資家の動きを検出
- **タートルトレーディング**: 伝説的なトレンドフォロー戦略、55日ブレイクアウト + ATRストップロス

**効果⭐⭐⭐⭐ の戦略（4種）**
- **KAMA（適応型移動平均）**: 市場状況に応じて自動調整、トレンド時は速くレンジ時は遅く反応
- **DMI（方向性指数）**: +DIと-DIのクロスオーバーでトレンド方向を判定
- **出来高急増**: 平均の2.5倍以上の出来高発生時にエントリー、大口の動きに追従
- **DEMA（二重EMA）**: EMAのEMAでラグを削減、素早いトレンド転換検出

**効果⭐⭐⭐ の戦略（3種）**
- **シャンデリアエグジット**: ATRベースのトレーリングストップ、最高値からATR×3で決済
- **3本移動平均**: 短期・中期・長期MAの完全整列（10/30/90日）でトレンド判定
- **週次ピボットポイント**: 週単位のピボットレベル、サポート/レジスタンスで売買
